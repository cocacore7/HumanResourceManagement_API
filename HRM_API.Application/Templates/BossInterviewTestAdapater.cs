...
{
    public class BossInterviewAdapter(IMailRepository repository, ISettings settings) : ITemplateRepository
    {
        private readonly IMailRepository _repository = repository;
        private readonly ISettings _settings = settings;

        public string TemplateName => "BossInterview.html";

        public async Task<string> BuildBodyAsync(int id)
        {
            var dto = await _repository.GetBossInterviewAsync(id) ?? throw new Exception($"No se encontró la prueba con id={id}");
