...
namespace HRM_API.Application.Services
{
    public class FormService(IFormRepository repository, IPreApplicationRepository preApplicationRepository, IFileRepository fileRepository, IUserRepository userRepository, FileHelper fileHelper, MailHelper mailHelper)
    {
        ...
        public async Task<SetFormAnswersReponseDto?> SetFormAnswersAsync(GeneralFormRequestDto request, LoginDBResponseDto user)
        {
            ...
            var emailRecruiterExt = await _userRepository.GetActualUserEmailAsync(preApplication.FirstOrDefault()?.Id ?? 0);
            if (preApplicationState == EnumHelper.GetEnumDescription(UpdateFormAnswersEmailStatusEnum.Interview) && request?.Origin.State == EnumHelper.GetEnumDescription(UpdateFormAnswersEmailStatusEnum.InterviewFail))
            {//Correo a Candidato
            ...
            {//Correo a Candidato
                await _mailHelper.SendEmailFromTemplateAsync(emailRecruiterExt ?? string.Empty, "Gestión en etapa de " + request?.Origin.State, "Record", int.TryParse(preApplication.FirstOrDefault()?.Id.ToString(), out int UserId) ? UserId : 0);
            }
                        ...
                        switch (item.Type)
                        {
                            ...
                            case var type when type == EnumHelper.GetEnumDescription(SetFormAnswersTypeFileEnum.File):
                                ...
                                if (!string.IsNullOrEmpty(item.Base64))
                                {
                                    ...
                                    responseFileId = (int)await _fileRepository.SetFileAsync(newfile);
                                }
        ...
        public async Task<UpdateFormAnswersReponseDto?> UpdateFormAnswersAsync(GeneralFormRequestDto request, LoginDBResponseDto user)
        {
            ...
            if (preApplication == null) { return new() { Response = ["No existe la Pre Aplicación solicitada"] }; }
            //Validar Registro PreApplicationFormResponse
            var reponseId = await _repository.GetPreApplicationFormResponseAsync(preApplication.FirstOrDefault()?.Id, form.IdForm);
